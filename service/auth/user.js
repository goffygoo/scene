...
import { generateOtp, generateRefreshToken, httpRequest, processPassword } from '../../util/index.js';
...
const { GOOGLE_TOKEN_URL, GOOGLE_CLIENT_ID_ANDROID, GOOGLE_CLIENT_SECRET_ANDROID } = config;

const loginOrganiser = async ({ body }) => {
    const { email, password } = body;
    const organiser = await Organiser.findOne({ email });
    if (!organiser || organiser.password !== processPassword(password)) throw Error();
    const userId = organiser._id;
    const refreshToken = organiser.refreshToken;
...
const googleLogin = async ({ body }) => {
    const { code } = body;
    const googleResponse = await httpRequest(
        "post",
        GOOGLE_TOKEN_URL,
        {
            code,
